...
///
/// @desc Properties accepted by the constructor of {@link FORMS_ApplicationSurface}.
function FORMS_ApplicationSurfaceProps(): FORMS_CompoundWidgetProps() constructor
{
	/// @var {Id.Surface, Undefined} An ID of a surface to draw instead of the application surface or `undefined` to
	/// draw the application surface.
	Surface = undefined;

	/// @var {Bool, Undefined} Whether to draw the surface stretched (`true`) or scaled while keeping its aspect ratio
	/// (`false`).
	Stretch = undefined;
...
///
/// @desc A collection of widget drawn on top of a surface (by default the application surface).
///
/// @param {Struct.FORMS_ApplicationSurfaceProps, Undefined} [_props] Properties to create the surface widget with or
/// `undefined` (default).
/// @param {Array<Struct.FORMS_Widget>, Undefined} [_children] An array of child widgets to add to the surface widget or
/// `undefined` (default).
...
/* beautify ignore:end */
{
	...
	/// @var {Id.Surface, Undefined} An ID of a surface to draw instead of the application surface or `undefined`
	/// (default) to draw the application surface.
	Surface = forms_get_prop(_props, "Surface");

	/// @var {Bool} Whether to draw the surface stretched (`true`) or scaled  while keeping its aspect ratio (`false`,
	/// default).
	...
	/// @var {Real} The absolute Y position of the drawn surface. Updated in [layout](./FORMS_Widget.layout.html).
	/// @readonly
	SurfaceY = 0;

	/// @var {Real} The actual width of the drawn surface after it's stretched or scaled. Updated in
	/// [layout](./FORMS_Widget.layout.html).
	/// @readonly
	SurfaceWidth = 0;

	/// @var {Real} The actual height of the drawn surface after it's stretched or scaled. Updated in
	/// [layout](./FORMS_Widget.layout.html).
	/// @readonly
	SurfaceHeight = 0;

	/// @var {Real} The X position of the mouse, relative to the surface's position. Updated in
	/// [layout](./FORMS_Widget.layout.html).
	/// @readonly
	MouseX = 0;

	/// @var {Real} The Y position of the mouse, relative to the surface's position. Updated in
	/// [layout](./FORMS_Widget.layout.html).
	/// @readonly
	MouseY = 0;
	...
	///
	/// @return {Id.Surface} The ID of the surface that the widget draws or -1 if
	/// {@link FORMS_ApplicationSurface.Surface} is `undefined` and the application surface is disabled.
	...
	/// @func is_mouse_over()
	///
	/// @desc Checks whether mouse position is inside of a rectangle defined by  the position and size of the surface
	/// that this widget draws.
	///
	/// @return {Bool} Returns `true` if mouse is inside of the rectangle.
	///
	/// @example
	/// The following example shows how to check whether the is mouse cursor inside of a surface drawn by a widget
	/// `viewport`, taking into account which widget is currently hovered (the top level widget underneath the mouse
	/// cursor must also be `viewport`).
	/// ```gml
	/// /// @desc Create event
