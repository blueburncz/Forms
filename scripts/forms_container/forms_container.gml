...
{
	/// @var {Asset.GMSprite, Undefined} The background sprite of the container, stretched over its entire size.
	...
	/// @var {Bool, Undefined} Whether the container should be automatically resized to fit its content upon its
	/// creation.
...
///
/// @param {Struct.FORMS_ContainerProps, Undefined} [_props] Properties to create the container with or `undefined`
/// (default).
function FORMS_Container(_props = undefined): FORMS_Widget(_props) constructor
{
	...
	/// @var {Asset.GMSprite} The background sprite of the container, stretched over its entire size. Defaults to
	/// `FORMS_SprRectangle`.
	...
	/// [layout](./FORMS_Widget.layout.html) is called. Defaults to `false`.
	ContentFit = forms_get_prop(_props, "ContentFit") ?? false;

	/// @var {Struct.FORMS_Pen} A pen used by this container to draw text and controls like inputs, checkboxes, buttons
	/// etc.
	/// @readonly
	...
	///
	/// @desc Draws the content into a off-screen surface to fetch its width and height.
	///
	...
	///
	/// @param {Real} _scroll The new scroll value. Clamped between 0 and the width of the container's content.
	///
	/// @return {Struct.FORMS_Container} Returns `self`.
	...
	///
	/// @param {Real} _scroll The new scroll value. Clamped between 0 and the height of the container's content.
	///
	/// @return {Struct.FORMS_Container} Returns `self`.
